...
function parseMultipart(req) {
  return new Promise((resolve, reject) => {
    ...
    const files = [];
    let oversized = 0;
    const bb = Busboy({headers:req.headers,limits:{files:MAX_FILES,fileSize:MAX_FILE_SIZE,fields:30}});
...
async function saveAnalysis(token,userId,fields,analysis,price,creditSource){
  ...
  if(!r.ok) throw new Error('Falha ao salvar análise no histórico.');
